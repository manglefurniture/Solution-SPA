...
if($method==='POST'){
  ...
  if($status==='paid'){$q=$pdo->prepare("SELECT id FROM payments WHERE appointment_id=:appointment AND status='paid' LIMIT 1");$q->execute(['appointment'=>$appointment]);if($q->fetchColumn())respond(['error'=>'Esta cita ya tiene un pago registrado'],409);}
