...
if ($method === 'PATCH') {
    ...
    $stmt = $pdo->prepare('SELECT id,name,email,role,client_id,active FROM users WHERE id=:id LIMIT 1');
    $stmt->execute(['id'=>$id]);
    $current = $stmt->fetch();
    if (!$current) respond(['error' => 'El usuario no existe'], 404);

    $name = trim((string)($data['name'] ?? $current['name']));
    $email = strtolower(trim((string)($data['email'] ?? $current['email'])));
    $role = validateRole((string)($data['role'] ?? $current['role']));
    $active = array_key_exists('active',$data) ? ((bool)$data['active'] ? 1 : 0) : (int)$current['active'];
    $clientId = validateClientLink($pdo, $role, $data['client_id'] ?? $current['client_id']);
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) respond(['error'=>'Correo inválido'],422);
    if ($id === (int)$admin['id'] && ($active !== 1 || $role !== 'admin')) respond(['error'=>'No puedes quitarte tu propio acceso de administrador'],409);
