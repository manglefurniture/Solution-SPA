...
if($method==='GET'){
    ...
    if($clientIdRaw!==''){$clientId=validEntityId($clientIdRaw);if($clientId===false)respond(['error'=>'Cliente inválido'],422);$where[]='a.client_id=:client_id';$params['client_id']=$clientId;$limit=min($limit,100);}
    if($where)$sql.=' WHERE '.implode(' AND ',$where);$sql.=' ORDER BY a.starts_at '.($clientIdRaw!==''?'DESC':'ASC').' LIMIT '.$limit;$stmt=$pdo->prepare($sql);$stmt->execute($params);respond(['data'=>$stmt->fetchAll()]);
}
