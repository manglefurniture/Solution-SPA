...
if ($method === 'GET') {
    ...
    if ($nextFrom !== '') {
        ...
        if (!$from) {
            respond(['error' => 'Invalid next_from'], 422);
        }
...
if ($method === 'POST') {
    ...
    if ($clientId === false || $serviceId === false) {
        respond(['error' => 'Cliente o servicio inválido'], 422);
    }

    $startsAt = strictDateTime(trim((string)$data['starts_at']));
    if (!$startsAt) {
        respond(['error' => 'Fecha u hora inválida'], 422);
    }

    $status = (string)($data['status'] ?? 'pending');
    $allowed = ['pending', 'confirmed', 'completed', 'cancelled'];
    if (!in_array($status, $allowed, true)) {
        respond(['error' => 'Estado inválido'], 422);
    }
    ...
    if (!$clientCheck->fetchColumn()) {
        respond(['error' => 'El cliente no existe'], 422);
    }
    ...
    if ($serviceActive === false) {
        respond(['error' => 'El servicio no existe'], 422);
    }
    if ((int)$serviceActive !== 1) {
        respond(['error' => 'El servicio está inactivo'], 422);
    }
