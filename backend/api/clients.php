...
function normalizeClient(array $data, array $fallback = []): array
{
    ...
    $phoneRaw = trim((string)($data['phone'] ?? $fallback['phone'] ?? ''));
    ...
    if ($name === '' || $phoneRaw === '') respond(['error' => 'Nombre y teléfono son obligatorios'], 422);
...
if ($method === 'GET') {
    ...
    $sql = 'SELECT id,name,phone,email,birth_date,notes,active,created_at FROM clients'
        . $whereSql . ' ORDER BY active DESC,name LIMIT ' . $perPage . ' OFFSET ' . $offset;
...
if ($method === 'POST') {
    ...
    auditMutation($pdo, $user, 'client.created', 'client', $id, null, $client);
    respond(['id' => $id], 201);
}
...
if ($method === 'PATCH') {
    ...
    if ($user['role'] === 'operator' && array_key_exists('active', $data) && (int)(bool)$data['active'] !== (int)$current['active']) {
        respond(['error' => 'Los gestores no pueden archivar ni reactivar clientes'], 403);
    }
    ...
    try {
        ...
        if ((int)$client['active'] !== (int)$current['active']) {
            $stmt = $pdo->prepare("UPDATE users SET active=:active WHERE client_id=:id AND role='client'");
            $stmt->execute(['active' => $client['active'], 'id' => $id]);
            if ((int)$client['active'] === 0) {
                $pdo->prepare('DELETE rt FROM remember_tokens rt JOIN users u ON u.id=rt.user_id WHERE u.client_id=:id')->execute(['id' => $id]);
            }
        }

        $action = (int)$client['active'] !== (int)$current['active']
            ? ((int)$client['active'] === 1 ? 'client.reactivated' : 'client.archived')
            : 'client.updated';
        $after = $client;
        unset($after['id']);
        auditMutation($pdo, $user, $action, 'client', $id, $current, $after);
        $pdo->commit();
        respond(['ok' => true]);
    } catch (Throwable $e) {
        if ($pdo->inTransaction()) $pdo->rollBack();
        respond(['error' => 'No se pudo actualizar el cliente'], 500);
    }
}
...
if ($method === 'DELETE') {
    ...
    $exists = $pdo->prepare('SELECT name,phone,email,birth_date,notes,active FROM clients WHERE id=:id LIMIT 1');
    ...
    try {
        $pdo->beginTransaction();
        $pdo->prepare('UPDATE clients SET active=0 WHERE id=:id')->execute(['id' => $id]);
        $pdo->prepare("UPDATE users SET active=0 WHERE client_id=:id AND role='client'")->execute(['id' => $id]);
        $pdo->prepare('DELETE rt FROM remember_tokens rt JOIN users u ON u.id=rt.user_id WHERE u.client_id=:id')->execute(['id' => $id]);
        ...
        auditMutation($pdo, $user, 'client.archived', 'client', $id, $current, ['active' => 0], ['cancelled_future_appointments' => $cancelledFuture]);
        $pdo->commit();
