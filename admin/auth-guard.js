(()=>{
  try{
    ...
    window.fetch=(input,init={})=>{
      const opts={...init};const method=String(opts.method||'GET').toUpperCase();
      if(['POST','PATCH','PUT','DELETE'].includes(method)){
        const url=typeof input==='string'?input:(input&&input.url)||'';
        if(!/^https?:\/\//i.test(url)||new URL(url,location.href).origin===location.origin){const headers=new Headers(opts.headers||{});headers.set('X-CSRF-Token',window.solutionCsrf);opts.headers=headers;}
      }
    ...
    window.addEventListener('DOMContentLoaded',()=>{
      ...
      const roleUi=document.createElement('script');roleUi.src='role-ui.js?v=20260824-rbac2';document.body.appendChild(roleUi);if(user.role==='admin'){const usersUi=document.createElement('script');usersUi.src='users-ui.js?v=20260824-rbac2';document.body.appendChild(usersUi);}
    });
